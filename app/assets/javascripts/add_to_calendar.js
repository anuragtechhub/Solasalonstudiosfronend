...
$.widget('solasalonstudios.add_to_calendar', {
  _create: function() {
    var self = this;
    var params = {data: {
    ...
    var hideCalendarLinks = function () {
      //console.log('hideCalendarLinks')
      $(window).off('click.add-to-calendar-links');
      self.element.find('.calendar-links-wrapper').hide();
    };

    var showCalendarLinks = function () {
      //console.log('showCalendarLinks');
      $(window).on('click.add-to-calendar-links', function () {
        hideCalendarLinks();
      });
      self.element.find('.calendar-links-wrapper').show();
    };


    self.element.append(self.generateMarkup(self.generateCalendars(params.data), self.getClass(params), self.getOrGenerateCalendarId(params)));
    self.element.find('.add-to-calendar-checkbox').on('click', function (e) {
      e.stopPropagation();
      e.preventDefault();

      if (self.element.find('.calendar-links-wrapper').is(':visible')) {
        hideCalendarLinks();
      } else {
        showCalendarLinks();
      }
  ...
  calculateEndTime: function (event) {
    ...
    return event.end ?
      self.formatTime(event.end) :
      self.formatTime(new Date(event.start.getTime() + (event.duration * self.MS_IN_MINUTES)));
  },
  ...
  generateCalendars: function (event) {
    var self = this;
  ...
  generateMarkup: function (calendars, clazz, calendarId) {
    ...
    Object.keys(calendars).forEach(function(services) {
      calendarHTML += calendars[services];
    });
    result.innerHTML += '<span class="calendar-links-wrapper">' + calendarHTML + '</span>';
  ...
  getOrGenerateCalendarId: function (params) {
