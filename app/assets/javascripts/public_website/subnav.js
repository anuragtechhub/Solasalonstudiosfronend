  ...
  */
  ...
  var showOysNav = function (nav) {
    if ($(window).width() > 1000) {
      ...
      $('.oys-nav').not('[data-nav="' + nav + '"]').slideUp('fast');
      $('.oys-nav[data-nav="' + nav + '"]').css({left: $headerContainer.offset().left, top: $headerContainer.outerHeight() + $('#top-header').outerHeight() + 5}).width($headerContainer.outerWidth()).slideDown('fast');
    }
  };
  ...
  $('.oys-nav').on('mouseenter', function (e) {
    ...
    $('.oys-nav[data-nav="' + showingNav + '"]').addClass('over').slideDown('fast');
  });  
  ...
  $('.oys-nav').on('mouseleave', function (e) {
    ...
    $('.oys-nav[data-nav="' + showingNav + '"]').removeClass('over').slideUp('fast');
    showingNav = null;
  }); 
