class Api::SolaCms::SideMenuItemsController < Api::SolaCms::ApiController
  ...
  #GET /side_menu_items
  def index
    @side_menu_items = params[:search].present? ? search_side_menu_items : SideMenuItem.all
  ...
  def create
    ...
    if @side_menu_item.save
      render json: @side_menu_item, status: 201
    else
  ...
  def search_side_menu_items
    SideMenuItem.search_by_side_menu_items(params[:search])
  end
end
