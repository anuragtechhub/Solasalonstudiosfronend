class Api::SolaCms::SolaClassRegionsController < Api::SolaCms::ApiController
  ...
  def index
    @regions = params[:search].present? ? search_sola_class_region : SolaClassRegion.order("#{field} #{order}")
  ...
  def create 
    ...
    if @region.save
      ...
      Rails.logger.error(@region.errors.messages)
  ...
  def search_sola_class_region
    SolaClassRegion.order("#{field} #{order}").search_regions(params[:search])
  end 
end
