...
class ContactUsController < PublicWebsiteController
  ...
  skip_before_action :verify_authenticity_token, only: %i[franchising_request request_a_tour]
  ...
  def request_a_tour
    if request.post?
      if params[:required_fields].present?
        ...
        params[:required_fields].each do |required_field|
          ...
          if required_field == 'email' && params[required_field].present? && is_valid_email?(params[required_field])
            # all good
          elsif params[required_field]&.present?
            # all good
          else
            all_good = false
            break
      ...
      if (params[:required_fields].present? && all_good) && (params[:name]&.present? && params[:email] && params[:email].present? && is_valid_email?(params[:email])) # && params[:message].present? && params[:contact_preference].present?
        # p "BOUT TO CHECK"
        ...
        elsif verify_recaptcha(response: params[:recaptcha_token], private_key: ENV.fetch('RECAPTCHA_SECRET_KEY', nil))
      ...
      elsif params[:required_fields].present?
        ...
        Rails.logger.debug { "params[:dont_see_your_location]=#{params[:dont_see_your_location]}" }
        Rails.logger.debug { "is_valid_zip_code?(params[:zip_code])=#{is_valid_zip_code?(params[:zip_code])}" }
      ...
      redirect_to :contact_us, status: :moved_permanently
    end
  rescue Gibbon::MailChimpError => e
    NewRelic::Agent.notice_error(e)
    Rollbar.error(e)
