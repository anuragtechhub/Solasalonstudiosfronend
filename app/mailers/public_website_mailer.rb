...
class PublicWebsiteMailer < ActionMailer::Base
  ...
  def modern_salon_2019_05(requestTourInquiry)
    ...
    if requestTourInquiry&.email && requestTourInquiry.email.present?
      @inquiry = requestTourInquiry
      # requestTourInquiry.location.email_address_for_inquiries
  ...
  def stylist_message(stylistMessage)
    ...
    if stylistMessage&.stylist && stylistMessage.stylist.email_address && stylistMessage.stylist.email_address.present?
      @message = stylistMessage
      mail(to: stylistMessage.stylist.website_email_address.presence || stylistMessage.stylist.email_address, from: 'Sola Salon Studios <inquiry@solasalonstudios.com>', subject: 'Sola Website Inquiry') # stylistMessage.stylist.email_address
    end
  end
  ...
  def stylist_website_is_updated(update_my_sola_website)
    ...
    if update_my_sola_website
      ...
      if @update_my_sola_website&.stylist && @update_my_sola_website.stylist.email_address
