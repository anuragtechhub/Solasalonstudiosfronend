class PublicWebsiteMailer < ActionMailer::Base
  ...
  def request_a_tour(requestTourInquiry)
    if requestTourInquiry && requestTourInquiry.location && requestTourInquiry.location.email_address_for_inquiries && requestTourInquiry.location.email_address_for_inquiries.present?
      ...
      mail(to: 'jeff@jeffbail.com', from: ("Sola Salon Studios <inquiry@solasalonstudios.com>"), subject: @inquiry.message.present? ? 'Sola Contact Us Inquiry' : 'Sola Rent a Studio Inquiry') #requestTourInquiry.location.email_address_for_inquiries
    end
  end
  ...
  def stylist_message(stylistMessage)
    if stylistMessage && stylistMessage.stylist && stylistMessage.stylist.email_address && stylistMessage.stylist.email_address.present?
      ...
      mail(to: 'jeff@jeffbail.com', from: "Sola Salon Studios <inquiry@solasalonstudios.com>", subject: 'Sola Website Inquiry') #stylistMessage.stylist.email_address
    end
  end
  ...
  def franchising_request(franchisingRequest)
    if franchisingRequest
      ...
      mail(to: 'jeff@jeffbail.com', from: "Sola Salon Studios <inquiry@solasalonstudios.com>", subject: 'Sola Franchising Request')
    end
  end
  ...
  def partner_inquiry(partnerInquiry)
    if partnerInquiry
      ...
      mail(to: 'jeff@jeffbail.com', from: "Sola Salon Studios <inquiry@solasalonstudios.com>", subject: 'Sola Partner Inquiry')
    end
  end
