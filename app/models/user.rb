...
class User < ActiveRecord::Base
  ...
  def generate_public_id
    Time.now.to_i.to_s + "#{SecureRandom.urlsafe_base64(3).gsub(/-|_/, '')}#{SecureRandom.hex(3)}".chars.shuffle.join
  end

  def set_public_id
    pid = generate_public_id
    pid = generate_public_id while User.where(public_id: pid).size.positive?
    self.public_id = pid
  end
end
