...
class BaseCli < Thor
  ...
  no_commands do
    ...
    def client_secrets_path
      return ENV.fetch('GOOGLE_CLIENT_SECRETS', nil) if ENV.key?('GOOGLE_CLIENT_SECRETS')
    ...
    def token_store_path
      return ENV.fetch('GOOGLE_CREDENTIAL_STORE', nil) if ENV.key?('GOOGLE_CREDENTIAL_STORE')
    ...
    def well_known_path_for(file)
      if OS.windows?
        dir = ENV.fetch('HOME') { ENV.fetch('APPDATA', nil) }
        File.join(dir, 'google', file)
      else
        File.join(ENV.fetch('HOME', nil), '.config', 'google', file)
      end
    end
    ...
    def user_credentials_for(scope)
      ...
      client_id = if ENV['GOOGLE_CLIENT_ID']
                    Google::Auth::ClientId.new(ENV.fetch('GOOGLE_CLIENT_ID', nil), ENV.fetch('GOOGLE_CLIENT_SECRET', nil))
                  else
    ...
    def api_key
      ENV.fetch('GOOGLE_API_KEY', nil) || options[:api_key]
    end
  end
end
