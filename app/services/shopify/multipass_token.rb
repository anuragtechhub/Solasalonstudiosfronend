...
class Shopify::MultipassToken
  SHOPIFY_HOST = "https://#{ENV['SHOPIFY_LOGIN']}:#{ENV['SHOPIFY_PASSWORD']}@solasalon.myshopify.com".freeze

  def initialize(user)
    ### Use the Multipass secret to derive two cryptographic keys,
    ### one for encryption, one for signing
    key_material = OpenSSL::Digest.new('sha256').digest(ENV['MULTIPASS_SECRET'])
  ...
  def all_tags
    (current_tags.blank? ? ['solapro-created'] : current_tags) | %w[approved solastylist solapro-user]
  end
