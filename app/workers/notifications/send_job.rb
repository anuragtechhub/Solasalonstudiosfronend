  ...
  class SendJob
    ...
    def perform(type, notification_id = nil, user_id = nil)
      ...
      when 'send'
        ...
        @notification.update(date_sent: DateTime.current)
      when 'update_badge'
    ...
    private
      ...
      def update_badge
        @user.devices.each do |device|
          ...
          Rails.logger.debug { "#{@user.email_address} have device token #{device.token}, update badge" }
          ...
          begin
            ...
            Rails.logger.debug { "#{@user.email_address} sns publish error #{e.inspect}" }
      ...
      def create_in_app_notifications(cls)
        cls.connection.execute <<-SQL.squish
      ...
      def sns
        @sns ||= Aws::SNS::Client.new(stub_responses: @stub_responses,
                                      region:         ENV.fetch('SNS_REGION', nil),
      ...
      def send_push_notifications
        @notification.user_notifications.reload.each do |user_notification|
          ...
          user.devices.each do |device|
            ...
            Rails.logger.debug { "#{user.email_address} DOES have device token #{device.token}, publish notification" }
            begin
              badge = user.user_notifications.where(dismiss_date: nil).count
              sns.publish(
                target_arn:        endpoint_arn(device.token, device.platform),
