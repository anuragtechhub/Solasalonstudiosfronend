  ...
  class Api::V1::VideosController < ApiController
    ...
    def history_load_more
      ...
      #json = Rails.cache.fetch(cache_key) do
        ...
        @videos = VideoView.where(:id => VideoView.select('DISTINCT ON (video_id) *').where(:userable_id => userable.id, :userable_type => userable.class.name).map{|v| v.id}).order(:created_at)
