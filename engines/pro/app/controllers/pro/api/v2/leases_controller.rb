  ...
  class Api::V2::LeasesController < ApiController
    ...
    def studio_agreement
      ...
      if userable.respond_to? :leases
        ...
        if userable.leases&.length&.positive?
          lease = userable.leases.order(created_at: :desc).first
          if lease && lease.agreement_file_url.present?
            ...
            payload = {
              ...
              'SpecialTerms'                => lease.special_terms.presence || '',
            ...
            if lease.insurance_amount.present? && lease.insurance
              ...
              payload['InsuranceFrequency'] = case lease.insurance_frequency
                                              when 'weekly'
                                                1
                                              when 'monthly'
                                                2
            ...
            if lease.move_in_bonus # && lease.move_in_bonus_payee.present?
              ...
              payload['MoveInBonusPayee'] = lease.move_in_bonus_payee.presence || ''
            end

            # recurring charges

            # rent
            if lease.rent_recurring_charges&.length&.positive?
            ...
            # tax
            if lease.tax_recurring_charges&.length&.positive?
            ...
            # parking
            if lease.parking_recurring_charges&.length&.positive?
            ...
            # cable
            if lease.cable_recurring_charges&.length&.positive?
            ...
            token = fill_agreement_tokens_json[0]['Token'] if fill_agreement_tokens_json&.length&.positive?
            p "token=#{token}"
            # token = 'denver'
