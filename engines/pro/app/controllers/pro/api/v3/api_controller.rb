...
module Pro
  class Api::V3::ApiController < ApiController
    ...
    def history_scope(scope)
      if params[:history].present?
      ...
      elsif params[:q].present? && scope.table_name == 'sola_classes'
        ...
        order = current_user.brands.exists? ? "CASE WHEN #{scope.table_name}.brand_id in (#{current_user.brands.ids.join(',')}) THEN 1 ELSE 0 END desc" : ''
        scope.order(order, "#{scope.table_name}.#{order_field} #{order_vector}")
      else
