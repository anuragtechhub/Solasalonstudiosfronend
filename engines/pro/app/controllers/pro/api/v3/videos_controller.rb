...
module Pro
  class Api::V3::VideosController < Api::V3::ApiController
    ...
    def index
      @videos = @videos.not_webinars if params[:suggested].blank?
      ...
      @videos = @videos.where(id: Video.includes(:categories).where(categories: { id: params[:category_id] }).select(:id)) if params[:category_id].present?
