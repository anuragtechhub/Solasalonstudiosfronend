module Pro
  class Api::V3::WebinarsController < Api::V3::ApiController
    load_and_authorize_resource class: 'Video', instance_name: :webinar, only: [:index, :show]
    ...
    def index
      ...
      @webinars = @webinars.where(id: Video.includes(:categories).where(categories: { id: params[:category_id] }).pluck(:id)) if params[:category_id].present?
