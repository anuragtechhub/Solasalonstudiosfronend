    ...
    csv_report = CSV.generate do |csv|
      ...
      months.each do |month|
        ...
        locations.each do |location|
          csv << [month.strftime('%B %Y'), location.name, location.store_id,  RequestTourInquiry.where('location_id = ? AND how_can_we_help_you != ? AND (created_at <= ? AND created_at >= ?)', location.id, "Book an appointment with a salon professional", month.end_of_month, month.beginning_of_month).count]
        end
      end
  ...
  def send_all_stylists_report(email_address=nil)
    ...
    csv_report = CSV.generate do |csv|
      csv << ['ID', 'First Name', 'Last Name', 'URL Name', 'Email Address', 'Phone Number', 'Website URL', 'Booking URL',
              ...
              'Studio Number', 'Location ID', 'Location Name', 'Location City', 'Location State', 'Country', 'Has Sola Pro',
      ...
      Stylist.where('status = ?', 'open').order(:created_at => :desc).each do |stylist|
        ...
        csv << power_of_now([stylist.id, stylist.first_name, stylist.last_name, stylist.url_name, stylist.email_address, stylist.phone_number, stylist.website_url, stylist.booking_url,
                ...
                stylist.studio_number, stylist.location.id, stylist.location.name, stylist.location.city, stylist.location.state, stylist.country, stylist.has_sola_pro_login,
                stylist.has_sola_genius_account, stylist.sola_pro_start_date, stylist.sola_pro_platform, stylist.sola_pro_version])
      end
    end

    ReportsMailer.send_report(email_address, 'All Stylists Report', csv_report).deliver
  end
